...
/// calibration tooling: `(label, white-perspective centipawns)` in the same
/// order `evaluate` sums them, plus the endgame scale factor applied at the
/// end (out of `SCALE_FACTOR_NORMAL`). Deliberately mirrors `evaluate`
/// instead of being called by it — `evaluate` runs at every search node and
/// must not allocate — with a test pinning the two to identical totals so
/// they can't drift apart silently.
pub fn evaluate_breakdown(board: &Board) -> (Vec<(&'static str, i32)>, i32) {
    let phase = game_phase(board);
    if phase == 0 {
        let total_pawns =
            board.pieces_of(Color::White, PieceType::Pawn).count() + board.pieces_of(Color::Black, PieceType::Pawn).count();
        if total_pawns == 1 {
            return (vec![("kpk_exact", kpk_exact_score(board))], SCALE_FACTOR_NORMAL);
        }
    }
    let occ = board.occupied();
    let white_attacks = attack_info_for(board, Color::White, occ);
    ...
    let terms = vec![
        ...
        ("pawn_structure", pawn_structure_score_with_phase(board, phase)),
        ("bishop_pair", bishop_pair_score(board)),
        ("bishop_pawns", bishop_pawns_score(board)),
        ("king_safety", king_safety_score_with(board, phase, &white_attacks, &black_attacks)),
        ("mop_up", mop_up_score_with_phase(board, phase)),
        ("rook_file", rook_file_score(board)),
        ("rook_seventh", rook_on_seventh_score(board)),
        ("knight_outpost", knight_outpost_score(board)),
        ("pawn_endgame", pawn_endgame_score(board, phase)),
...
fn non_pawn_material(board: &Board, color: Color) -> i32 {
    PieceType::ALL
        .iter()
        .filter(|&&kind| !matches!(kind, PieceType::Pawn | PieceType::King))
        .map(|&kind| PIECE_VALUES[kind as usize] * board.pieces_of(color, kind).count() as i32)
        .sum()
}

/// "Winnability" of a material lead held by a side with no pawns left: with
/// at most a minor piece of advantage, that side can never force a win —
/// even sweeping every enemy man off the board ends in insufficient mating
/// material, and a lone minor can't even do the sweeping. Found empirically
/// in this project's oracle calibration (2026-07-17): K+B vs K+P endings,
/// which the raw material count scores at +330, are dead draws for every
/// reference engine consulted, and this family alone accounted for the
/// largest measured miscalibration of the whole eval (+152 cp mean bias in
/// pawnless-phase endings). A lead of exactly a minor with nothing else on
/// the board scales almost to zero; a bigger own army with a still-small
/// *advantage* (e.g. KR vs KB) keeps a residual pull, matching how drawish
/// but not hopeless those endings are in practice.
...
fn pawnless_drawish_scale(board: &Board) -> Option<i32> {
    ...
    let advantage = strong_npm - non_pawn_material(board, strong.opposite());
    let minor = PIECE_VALUES[PieceType::Bishop as usize];
    if advantage > minor {
        return None;
    }
...
fn pawn_race_score(board: &Board) -> i32 {
    ...
    match (white_runs_free, black_runs_free) {
        ...
        (true, true) => {
            ...
            // Compared in actual plies, tempo included: with White to move,
            // White queens on ply 2w-1 and Black on ply 2b; with Black to
            // move, 2w vs 2b-1. `adjusted` is Black's ply count minus
            // White's, always odd — rightly so, since in a clean two-runner
            // race somebody always queens strictly first. (The previous
            // ad-hoc `margin - 1` tempo adjustment gave White no credit for
            // winning an equal-advance race on the move, while the mirrored
            // position did credit Black: an eval color asymmetry.)
            let adjusted = 2 * margin + if white_to_move { 1 } else { -1 };
            if adjusted > 0 {
                RACE_WIN_BONUS + adjusted.min(RACE_TEMPO_CAP) * RACE_TEMPO_WEIGHT
            } else {
                -(RACE_WIN_BONUS + (-adjusted).min(RACE_TEMPO_CAP) * RACE_TEMPO_WEIGHT)
            }
        }
...
mod tests {
    ...
    /// the en passant rank mirrored. The position is chess-identical with
    /// the colors exchanged, so `evaluate` must be exactly antisymmetric
    /// under it — see `evaluate_is_antisymmetric_under_color_mirroring`.
    fn color_mirrored_fen(fen: &str) -> String {
    ...
    #[test]
    fn evaluate_is_antisymmetric_under_color_mirroring() {
        // The one structural property a from-White's-perspective eval must
        // hold everywhere: swapping the colors of an arbitrary position
        // negates its score exactly. Any per-color term with mismatched
        // ranks/directions/tempo handling breaks this — it's what caught
        // the pawn-race tempo asymmetry this test was added alongside.
        let fens = [
            crate::board::STARTPOS_FEN,
            // Kiwipete: castling rights, pins, promotions in the air.
            "r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq - 0 1",
            // Equal two-runner pawn race, White to move.
            "8/8/6k1/P7/7p/8/8/K7 w - - 0 1",
            // Perft position 3: rook-and-pawns endgame with en passant themes.
            "8/2p5/3p4/KP5r/1R3p1k/8/4P1P1/8 w - - 0 1",
            // Middlegame with asymmetric material and exposed kings.
            "r4rk1/1pp1qppp/p1np1n2/2b1p1B1/2B1P1b1/P1NP1N2/1PP1QPPP/R4RK1 w - - 0 10",
            // Opposite-colored-bishops ending (exercises the scale factor).
            "8/5pk1/3b2p1/8/8/1B4P1/5PK1/8 w - - 0 1",
            // Queen vs pawn, Black to move.
            "4k3/8/8/3p4/8/8/8/3QK3 b - - 0 1",
        ];
        for fen in fens {
            let mirrored_fen = color_mirrored_fen(fen);
            let board = Board::from_fen(fen).unwrap();
            let mirrored = Board::from_fen(&mirrored_fen).unwrap();
            assert_eq!(
                evaluate(&board),
                -evaluate(&mirrored),
                "evaluate is color-asymmetric for {fen} (mirror: {mirrored_fen})"
            );
        }
    }

    #[test]
    fn evaluate_breakdown_sums_to_exactly_what_evaluate_returns() {
        // The breakdown deliberately duplicates `evaluate`'s term list (see
        // its doc comment for why); this is the tripwire that catches the
        // two drifting apart when a term is added to one but not the other.
        let fens = [
            crate::board::STARTPOS_FEN,
            "r3k2r/p1ppqpb1/bn2pnp1/3PN3/1p2P3/2N2Q1p/PPPBBPPP/R3K2R w KQkq - 0 1",
            "8/8/6k1/P7/7p/8/8/K7 w - - 0 1",
            "8/5pk1/3b2p1/8/8/1B4P1/5PK1/8 w - - 0 1",
            "8/8/8/4k3/4P3/8/8/K7 b - - 0 1", // KPK exact path
        ];
        for fen in fens {
            let board = Board::from_fen(fen).unwrap();
            let (terms, scale) = evaluate_breakdown(&board);
            let total: i32 = terms.iter().map(|(_, v)| v).sum();
            assert_eq!(
                (total * scale) / SCALE_FACTOR_NORMAL,
                evaluate(&board),
                "breakdown out of sync with evaluate for {fen}"
            );
        }
    }
    ...
    #[test]
    fn pawn_race_score_credits_the_side_to_move_in_an_equal_race() {
        // Both passers need 3 pushes and neither king can catch the other's
        // runner; White, being on the move, queens first and must be
        // credited for it. Regression test: the old tempo adjustment scored
        // this exact case 0 for White while the mirrored position (Black to
        // move) did credit Black — an eval color asymmetry.
        let board = Board::from_fen("8/8/6k1/P7/7p/8/8/K7 w - - 0 1").unwrap();
        assert!(
            pawn_race_score(&board) > 0,
            "White to move wins the equal race and must get the bonus, got {}",
            pawn_race_score(&board)
        );
    }
    ...
    #[test]
    fn a_lone_bishop_against_a_pawn_is_scaled_to_near_draw() {
        // The exact family the oracle calibration flagged: K+B vs K+P is a
