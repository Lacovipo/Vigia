...
impl Board {
    ...
    pub fn from_fen(fen: &str) -> Result<Board, String> {
        ...
        // The en passant target can only ever sit on rank 6 (White to move,
        // Black just double-pushed) or rank 3 (the reverse). Anything else
...
mod tests {
    ...
    fn from_fen_keeps_castling_rights_that_match_the_board() {
        ...
        assert_eq!(board.castling, CastlingRights::from_fen_str("KQkq").unwrap());
    }

    #[test]
    fn from_fen_rejects_garbage_castling_tokens() {
        // The old parser just extracted whichever KQkq letters appeared, so
        // both of these used to pass silently.
        assert!(Board::from_fen("4k3/8/8/8/8/8/8/4K3 w KXq - 0 1").is_err());
        assert!(Board::from_fen("4k3/8/8/8/8/8/8/4K3 w KK - 0 1").is_err());
    }

    #[test]
    fn from_fen_rejects_non_numeric_clock_fields() {
        assert!(Board::from_fen("4k3/8/8/8/8/8/8/4K3 w - - xx 1").is_err());
        assert!(Board::from_fen("4k3/8/8/8/8/8/8/4K3 w - - 0 yy").is_err());
        // Absent clock fields are still tolerated (EPD-style four-field FEN).
    ...
    #[test]
    fn from_fen_rejects_an_en_passant_square_on_an_impossible_rank() {
        // e4 can never be an en passant target for anyone...
        assert!(Board::from_fen("4k3/8/8/8/4P3/8/8/4K3 b - e4 0 1").is_err());
        // ...and e3 belongs to Black capturing, not White to move.
        assert!(Board::from_fen("4k3/8/8/8/4P3/8/8/4K3 w - e3 0 1").is_err());
    }
