...
/// every bump, and drifted).
const ENGINE_NAME: &str = concat!("Vigia ", env!("CARGO_PKG_VERSION"));
...
pub struct Engine {
    ...
    /// Persistent state behind `setoption name Ponder` (announced in
    /// `cmd_uci`): whether the GUI declared it may ponder at all. `go
    /// ponder` only withholds `bestmove` when this is set — a stray `go
    /// ponder` from a GUI that never enabled the option (and so will never
...
/// number against theirs on an identical `position`. One line per eval
/// term first (they also feed the calibration tooling), then the total in
/// the original single-line format, kept last and unchanged so existing
/// scripts that only want the number keep working.
fn cmd_eval(engine: &Engine, out: &mut impl Write) {
    let (terms, scale) = eval::evaluate_breakdown(&engine.board);
    for (label, value) in &terms {
        let _ = writeln!(out, "{label:>16}: {value:+5}");
    }
    if scale != 64 {
    ...
    while i < tokens.len() {
        match tokens[i] {
            ...
            "mate" => {
                ...
                // rather than turning it into a zero-depth search.
                limits.mate_in = tokens.get(i + 1).and_then(|s| s.parse().ok()).filter(|&n| n >= 1);
                i += 2;
            }
            _ => i += 1,
        }
...
fn cmd_go(engine: &mut Engine, tokens: SplitWhitespace) {
    ...
    // Gated on `ponder_enabled` (see that field's doc) so a stray `go
    // ponder` from a GUI that never opted into pondering can't withhold
    // `bestmove` forever waiting for a `ponderhit` that will never come.
    engine.pondering.store(limits.ponder && engine.ponder_enabled, Ordering::Relaxed);
...
fn spawn_search(
    ...
    thread::spawn(move || {
        ...
        // The periodic `info` lines above only counted the main thread's
        // own nodes (helpers stay silent, and sharing a live counter isn't
        // worth the synchronization); one final line reports the true
        // total so the GUI's last-seen nodes/nps reflect all threads —
        // and, when a helper reached a deeper completed iteration than
        // anything reported so far, also brings the last-seen
        // depth/score/pv in line with the move actually played.
        best.nodes = total_nodes;
        if best.depth > main_result.depth || total_nodes > main_result.nodes {
...
fn print_info(output: &mut impl Write, result: &search::SearchResult, elapsed: Duration) {
    ...
    let score_str = if result.score.abs() >= search::MATE_THRESHOLD {
...
mod tests {
    ...
    fn cmd_ponderhit_clears_the_pondering_flag() {
        let mut engine = Engine::new();
        let mut out = Vec::new();
        handle_command("setoption name Ponder value true", &mut engine, &mut out);
    ...
    #[test]
    fn setoption_ponder_toggles_the_persistent_flag() {
        let mut engine = Engine::new();
        let mut out = Vec::new();
        assert!(!engine.ponder_enabled);
        handle_command("setoption name Ponder value true", &mut engine, &mut out);
        assert!(engine.ponder_enabled);
        handle_command("setoption name Ponder value false", &mut engine, &mut out);
        assert!(!engine.ponder_enabled);
    }

    #[test]
    fn go_ponder_without_the_ponder_option_enabled_does_not_withhold_bestmove() {
        // A GUI that never sent `setoption name Ponder value true` will
        // never send `ponderhit` either: a stray `go ponder` must degrade
        // to a normal search instead of holding `bestmove` forever.
        let mut engine = Engine::new();
        let mut out = Vec::new();
        handle_command("go ponder depth 1", &mut engine, &mut out);
        assert!(!engine.pondering.load(Ordering::Relaxed));
        handle_command("quit", &mut engine, &mut out);
    }

    #[test]
    fn go_mate_is_parsed_into_mate_in() {
        let board = Board::start_pos();
        let limits = parse_go_limits("mate 3".split_whitespace(), &board);
        assert_eq!(limits.mate_in, Some(3));
        // Nonsense arguments are dropped rather than misparsed.
        let limits = parse_go_limits("mate 0".split_whitespace(), &board);
        assert_eq!(limits.mate_in, None);
        let limits = parse_go_limits("mate xx".split_whitespace(), &board);
        assert_eq!(limits.mate_in, None);
    }
}
